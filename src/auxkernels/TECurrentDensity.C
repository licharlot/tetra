...
#include "TECurrentDensity.h"

registerMooseObject("TetraApp", TECurrentDensity);
registerMooseObject("TetraApp", ADTECurrentDensity);

template <bool is_ad>
InputParameters
TECurrentDensityTempl<is_ad>::validParams()
{
...
template <bool is_ad>
TECurrentDensityTempl<is_ad>::TECurrentDensityTempl(const InputParameters & parameters)
...
template <bool is_ad>
RealVectorValue
TECurrentDensityTempl<is_ad>::computeValue()
{

  return MetaPhysicL::raw_value(_conductivity[_qp]) * -_grad_potential[_qp];
}

template class TECurrentDensityTempl<false>;
template class TECurrentDensityTempl<true>;
